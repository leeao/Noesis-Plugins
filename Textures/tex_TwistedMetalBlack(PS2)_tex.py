...
def noepyLoadRGBA(data, texList):
    ...
    while(bs.tell()<bs.getSize()):
        ...
        if tex.chunkFlag == 8:
            if(bs.tell()<bs.getSize()):
                ...
                if texPal.chunkFlag == 0x88:
                    if texPal.colors == 256:
                        ...
                        texName = "%d_8bpp_x%x_x%x"%(i,tex.texType,tex.chunkFlag)
                        ...
                        texName = "%d_4bpp_x%x_x%x"%(i,tex.texType,tex.chunkFlag)
                        texList.append(NoeTexture(texName, tex.width, tex.height, textureData, noesis.NOESISTEX_RGBA32))
                else:
                    ...
                    texName = "%d_32bpp_x%x_x%x"%(i,tex.texType,tex.chunkFlag)
            ...
            texName = "%d_A8_x%x_x%x"%(i,tex.texType,tex.chunkFlag)
            texList.append(NoeTexture(texName, tex.width,tex.height, textureData, noesis.NOESISTEX_RGBA32))
        elif tex.chunkFlag & 0x40 == 0x40:
            textureData = rapi.imageDecodeRaw(tex.pixel,tex.width,tex.height,"a4")
            texName = "%d_A4_x%x_x%x"%(i,tex.texType,tex.chunkFlag)
            texList.append(NoeTexture(texName, tex.width,tex.height, textureData, noesis.NOESISTEX_RGBA32))
        i += 1    
    return 1
